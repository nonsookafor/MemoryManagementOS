...
void print(list_t * l) {
    node_t* curr = l->head;
    while (curr != NULL) {
        printf("%d/%d->", curr->blk->start, curr->blk->end);
        curr = curr->next;
    }
    printf("None");
}
    ...
    */
		...
		if ((current->blk->end - current->blk->start) >= blocksize) {
				...
				while (current != NULL && current->next != NULL) {
						if ((current->next->blk->end - current->next->blk->start) >= blocksize) {
								p = current->next->blk;
				...
				if (p == NULL && (current->blk->end - current->blk->start) >= blocksize) {
						p = current->blk;
				}
		}
		...
		if (p == NULL) {
				...
				int original_end = p->end;
				p->pid = pid;
				p->end = p->start + blocksize - 1;
				list_add_ascending_by_address(alloclist, p);
				int amount = (current->blk->end - current->blk->start) - blocksize; 
				if (amount) {
						block_t *fragment = malloc(sizeof(block_t));
						fragment->pid = 0;
						fragment->start = p->end + 1;
    ...
    */
		...
		if (p == NULL) {
				...
				if (policy == 1) {
					...
					list_add_to_back(freelist, p);
				}
				else if (policy == 2) {
					list_add_ascending_by_blocksize(freelist, p);
				}
				else {
					list_add_descending_by_blocksize(freelist, p);
				}
		}
}
